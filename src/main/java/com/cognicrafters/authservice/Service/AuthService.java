...
import com.cognicrafters.authservice.Config.Credentials;
...
public class AuthService {
    ...
    public boolean logoutUser(String userId) {
        try {
            KeycloakConfig.getKeycloakInstance().realm("CogniCrafters").users().get(userId).logout();
            return true;
        } catch (Exception e) {
            e.printStackTrace();
            return false;
...
public void signup(User user) {
    ...
    if (response.getStatus() == Response.Status.CREATED.getStatusCode()) {
        ...
        // Vous pouvez également récupérer l'ID de l'utilisateur nouvellement créé comme ceci :
        String userId = CreatedResponseUtil.getCreatedId(response);
        System.out.println("User ID: " + userId);
    } else {
        // Échec de la création de l'utilisateur
        System.out.println("Failed to add user. Status: " + response.getStatus());
        // Pour plus de détails sur l'erreur, vous pouvez lire l'entité de réponse
    ...
    public boolean deleteUser(String userId) {
        try {
            KeycloakConfig.getKeycloakInstance().realm("CogniCrafters").users().delete(userId);
            return true;
        } catch (Exception e) {
            e.printStackTrace();
            return false;
...
//        try {
//            String userId = KeycloakConfig.getKeycloakInstance().realm("CogniCrqfters").users().search(email).get(0).getId();
//            CredentialRepresentation newCredential = new CredentialRepresentation();
//            newCredential.setType(CredentialRepresentation.PASSWORD);
//            newCredential.setValue(newPassword);
//            newCredential.setTemporary(false);
//            KeycloakConfig.getKeycloakInstance().realm("CogniCrafters").users().get(userId).resetPassword(newCredential);
//            return true;
//        } catch (Exception e) {
//            return false;
//        }
...
public boolean updatePassword(String email, String currentPassword, String newPassword) {
    try {
        Keycloak keycloak = KeycloakConfig.getKeycloakInstance();
        UsersResource usersResource = keycloak.realm("CogniCrafters").users();
